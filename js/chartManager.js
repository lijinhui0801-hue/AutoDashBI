...
const ChartManager = {
    ...
    renderChart: function (domId, type, data = [], config = {}) {
        ...
        if (typeof window.echarts === 'undefined') {
            this.destroyChart(domId);
            dom.innerHTML = this.createUnavailableMarkup(config);
            return;
        }
    ...
    createUnavailableMarkup: function (config = {}) {
        const title = this.escapeHtml(config.title || '图表组件');
        return `
            <div style="height:100%;display:flex;align-items:center;justify-content:center;padding:24px;background:linear-gradient(180deg, rgba(15,22,40,0.92), rgba(10,16,30,0.96));">
                <div style="width:100%;max-width:320px;padding:18px 20px;border-radius:18px;border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.04);text-align:center;">
                    <div style="font-size:15px;font-weight:600;color:#f7f9ff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${title}</div>
                    <div style="margin-top:8px;font-size:12px;line-height:1.6;color:rgba(236,242,255,0.62);">图表引擎加载失败，页面已切换为安全占位模式。</div>
                    <div style="margin-top:10px;font-size:11px;line-height:1.6;color:rgba(236,242,255,0.44);">如果你刚发布到 GitHub，通常是 CDN 超时或被拦截；刷新页面后会自动重试其它源。</div>
                </div>
            </div>
        `;
    },
